...
export async function GET() {
  try {
    const response = await workerProxyFetch("/tools/catalog", { method: "GET" });
    return NextResponse.json(await response.json(), { status: response.status });
  } catch (error) {
    if (error instanceof WorkerProxyError) return NextResponse.json({ detail: error.message }, { status: error.status });
    return NextResponse.json({ detail: "Unexpected error while loading the tool catalog." }, { status: 500 });
  }
}
